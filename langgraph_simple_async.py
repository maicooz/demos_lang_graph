#!/usr/bin/env python3
"""
LangGraph Async Document Processing Workflow (Simple Parsing)

This module implements a proper LangGraph async workflow with three agents:
1. Extractor Agent: Extracts entities using simple regex parsing (no LLM)
...
State format matches user's examples exactly.
"""
...
class AgentState(TypedDict):
    """State for the async document processing workflow - matches user's examples."""
    document: str
...
class LangGraphDocumentProcessor:
    """LangGraph async document processing workflow with simple parsing."""
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        # Compile the workflow without checkpointer for simple async execution
    ...
    async def process_document(self, document: str) -> Dict[str, Any]:
        """Process a document through the async LangGraph workflow."""
        # Initialize state exactly as in user's examples
...
async def main():
    """Example usage with the provided scenarios - matches user's code structure."""
    ...
    print("🚀 Testing LangGraph Async Workflow with User Scenarios\n")
    ...
    for i, input_doc in enumerate(scenarios, 1):
        ...
        # Process document using the exact pattern from user's examples
        initial_state = {
            "document": input_doc,
            "entities": {},
